...
class CommonsenseQAProcessor(DataProcessor):
  ...
  def _create_examples(self, lines, set_type):
    ...
    for  i, line in enumerate(lines):
      ...
      answers = np.array([
        tokenization.convert_to_unicode(line['choice0']),
      ...
      label = np.argwhere(answers == line['correct_answer'])
      assert len(label) == 1
      label = label[0][0]
...
def main(_):
  ...
  processor = CommonsenseQAProcessor(split=FLAGS.split, annotator_idx=FLAGS.annotator_idx, augment_ratio = FLAGS.augment_ratio, take_number=FLAGS.take_number)
