"""Run BERT on OpenBookQA for annotator ID prediction."""
...
flags.DEFINE_string(
  "split", None,
  "The split you'd like to run on, either 'annotator' or 'rand'.")
...
class CommonsenseQAProcessor(DataProcessor):
  ...
  def __init__(self, split, annotator_idx, augment_ratio, take_number):
  ...
  def get_labels(self):
    """See base class."""
    # These are the anonymized IDs of the top 5 annotator of OBQA,
    # as appear in the released dataset.
    return ["b356d338b7", "7b06152ffc", "dc78319bb0",
            "1e6dc77bb6", "cee82219a0", "OTHER"]
  ...
  def _create_examples(self, lines, set_type):
    ...
    for i, line in enumerate(lines):
        ...
        answers = ' ; '.join(
            [line['choice{}'.format(i)] for i in range(4)] + [line['correct_answer']]
        )

        text_a = tokenization.convert_to_unicode(line['question'])
        text_b = tokenization.convert_to_unicode(answers)

        if set_type == "test":
            label = "OTHER"
        else:
            if line["turkIdAnonymized"] in labels:
                label = tokenization.convert_to_unicode(line["turkIdAnonymized"])
            else:
                label = "OTHER"
        examples.append(
            InputExample(guid=qid, text_a=text_a, text_b=text_b, label=label))
